...
function initScene(config, seed) {
  ...
  function generateBody() {
    const style      = Math.random() < 0.2 ? 'banded' : 'smooth';
    ...
    if (style === 'banded') {
      ...
      while (y < 1.1) {
        ...
        bands.push({
          ...
          freq:     (0.8 + Math.random() * 2.0) * Math.PI * 2,
  ...
  function generateDust() {
    ...
    for (let i = 0; i < count; i++) {
      blobs.push({
        ...
        op: 0.008 + Math.random() * 0.008,
      });
    }
    return {
      blobs,
      hueShift:   (Math.random() - 0.5) * 30,
      haloRadius: 1.8 + Math.random() * 0.6,
      haloOp:     0.009 + Math.random() * 0.008,
    };
  }
